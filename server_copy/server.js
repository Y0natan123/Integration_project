...
// Ensure data directories exist for per-task DBs (prioritize collectDB/data)
const collectDbDataDir = path.join(__dirname, 'collectDB', 'data');
const legacyDataDir = path.join(__dirname, 'data');
if (!fs.existsSync(collectDbDataDir)) fs.mkdirSync(collectDbDataDir, { recursive: true });
if (!fs.existsSync(legacyDataDir)) fs.mkdirSync(legacyDataDir, { recursive: true });

// Use collectDB data directory as primary
const dataDir = collectDbDataDir;
    ...
    if (err) {
        ...
        `, (err) => {
            ...
            `, (err) => {
                ...
                // Add run_id column if it doesn't exist (for existing databases)
                mainDb.run(`ALTER TABLE Notes ADD COLUMN run_id INTEGER`, (err) => {
                    // Ignore error if column already exists
                    if (err && !err.message.includes('duplicate column name')) {
                        console.error('Error adding run_id column:', err.message);
                    } else if (!err) {
                        console.log('Added run_id column to Notes table');
                    }
...
// Get runs for a specific task - works with collectDB real data
app.get('/api/tasks/:id/runs', (req, res) => {
    const { id } = req.params;
    console.log(`Fetching runs for task ${id} from collectDB`);
    ...
    checkDb.get('SELECT id, name FROM Tasks WHERE id = ?', [id], (err, task) => {
        checkDb.close();
        ...
        // First try collectDB data directory
        const collectDbPath = path.join(__dirname, 'collectDB/data');
        
        // Look for matching collectDB files
        if (fs.existsSync(collectDbPath)) {
            const files = fs.readdirSync(collectDbPath);
            const taskFiles = files.filter(f => f.startsWith(`task_${id}_`) && f.endsWith('.db'));
            
            if (taskFiles.length > 0) {
                const taskDbPath = path.join(collectDbPath, taskFiles[0]);
                console.log(`Found collectDB file: ${taskDbPath}`);
                
                // Get runs from collectDB file
                const taskDb = new sqlite3.Database(taskDbPath, sqlite3.OPEN_READONLY);
                
                taskDb.all(`
                    SELECT 
                        time_point as run_id,
                ...
                `, (err, runs) => {
                    ...
                    if (err) {
                        console.error(`Error fetching runs from collectDB for task ${id}: ${err.message}`);
                        return res.status(500).json({ error: `Database error: ${err.message}` });
                    }
                    
                    console.log(`Successfully retrieved ${runs ? runs.length : 0} runs from collectDB for task ${id}`);
                    res.json(runs || []);
                });
                return;
            }
        ...
        // Fallback to server data directory (same logic as original server)
        const sanitizeName = (name) => name.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
        const taskDbPath = path.join(__dirname, 'data', `task_${id}_${sanitizeName(task.name)}.db`);
        
        // Check if task database exists
        if (!fs.existsSync(taskDbPath)) {
            console.log(`No data file found for task ${id}, returning empty runs`);
            return res.json([]);
        }
        
        // Get runs from task database
        const taskDb = new sqlite3.Database(taskDbPath, sqlite3.OPEN_READONLY);
        ...
        `, (err, runs) => {
            ...
            if (err) {
                console.error(`Error fetching runs for task ${id}: ${err.message}`);
                return res.status(500).json({ error: `Database error: ${err.message}` });
            }
            
            console.log(`Successfully retrieved ${runs ? runs.length : 0} runs for task ${id}`);
            res.json(runs || []);
        });
    });
});

// Get notes for a specific run
app.get('/api/tasks/:taskId/runs/:runId/notes', (req, res) => {
    const { taskId, runId } = req.params;
    console.log(`Fetching notes for task ${taskId}, run ${runId}`);
    
    // Validate parameters
    if (!taskId || isNaN(parseInt(taskId)) || !runId || isNaN(parseInt(runId))) {
        console.error(`Invalid parameters: taskId=${taskId}, runId=${runId}`);
        return res.status(400).json({ error: 'Invalid task ID or run ID' });
    }
    
    // Get notes from main database
    const db = new sqlite3.Database(path.join(__dirname, 'absorbanceDB.db'), sqlite3.OPEN_READONLY);
    
    db.all(`SELECT * FROM Notes WHERE task_id = ? AND run_id = ? ORDER BY created_at DESC`, [taskId, runId], (err, notes) => {
        db.close();
        
        if (err) {
            console.error(`Error fetching notes for task ${taskId}, run ${runId}: ${err.message}`);
            return res.status(500).json({ error: `Database error: ${err.message}` });
        }
        
        console.log(`Successfully retrieved ${notes ? notes.length : 0} notes for task ${taskId}, run ${runId}`);
        res.json(notes || []);
    });
});

// Add a note for a specific run
app.post('/api/tasks/:taskId/runs/:runId/notes', (req, res) => {
    const { taskId, runId } = req.params;
    const { content } = req.body;
    console.log(`Adding note for task ${taskId}, run ${runId}: ${content}`);
    ...
    // Add note to main database
    const db = new sqlite3.Database(path.join(__dirname, 'absorbanceDB.db'));
    
    db.run(
        `INSERT INTO Notes (task_id, run_id, content, created_at) VALUES (?, ?, ?, datetime('now'))`,
        [taskId, runId, content.trim()],
        function(err) {
            if (err) {
                db.close();
                console.error(`Error adding note for task ${taskId}, run ${runId}: ${err.message}`);
                return res.status(500).json({ error: `Database error: ${err.message}` });
            }
            ...
            db.get(`SELECT * FROM Notes WHERE id = ?`, [noteId], (err, note) => {
                ...
                console.log(`Note added successfully for task ${taskId}, run ${runId}`);
                res.status(201).json(note);
            });
        }
    );
